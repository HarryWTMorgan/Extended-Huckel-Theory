...
Input file for a 1D carbon chain with one atom in the unit cell
"""
...
def create_unit_cell():
    ...
    basis = np.array([["1","C1","2s",2,0,0,1.6,-0.7144],\
                     ["2","C1","2pz",2,1,0,1.8,-0.3921],\
                     ["3","C1","2px",2,1,1,1.8,-0.3921],\
                     ["4","C1","2py",2,1,-1,1.8,-0.3921]])
...
def generate_1D_supercell(atoms,positions,basis,a):
    ...
    This does not currently work properly for systems with multiple basis functions
    on one atom (the if/else statement below fixes it for the 1D carbon chain only)
    because it doesn't assign atom labels (new_basis[entry][1]) properly.
    """
    ...
    for entry in range(len(new_basis)):
        ...
        if entry < len(new_basis)/2 :
            new_basis[entry][1] = atom_list[0]
        else:
            new_basis[entry][1] = atom_list[1]
